...
import com.api.forohub.curso.CursoRepository;
import com.api.forohub.topico.*;
import com.api.forohub.curso.Curso;
import com.api.forohub.usuario.Usuario;
import com.api.forohub.usuario.UsuarioRepository;
import com.api.forohub.validaciones.ValidadorDeTopicos;
...
public class TopicoController {
    ...
    @Operation(summary = "Listar todos los tópicos activos",
            ...
            tags = { "Tópicos"})
    public ResponseEntity<Page<ListadoDeTopicos>> listadoTopicos(@PageableDefault(size = 10) Pageable paginacion) {
        return ResponseEntity.ok(topicoRepository.findAllByStatusTrue(paginacion).map(ListadoDeTopicos::new));
    }
    ...
    @Operation(summary = "Listar tópicos filtrados por nombre de curso y año",
            ...
            tags = { "Tópicos"})
    public ResponseEntity<Page<ListadoDeTopicos>> listarPorCursoYYear(
            @RequestParam String cursoNombre,
            @RequestParam int year,
            @PageableDefault(size = 10) Pageable paginacion) {
        Page<ListadoDeTopicos> topicos = topicoRepository.findByCursoNombreAndYear(cursoNombre, year, paginacion).map(ListadoDeTopicos::new);
