...
import com.api.forohub.respuesta.*;
...
public class RespuestaController {
    ...
    @Operation(summary = "Registra un nueva respuesta",
            description = "",
            tags = { "Respuestas"})
    public ResponseEntity<DatosRespuesta> nuevaRespuesta(@RequestBody @Valid NuevaRespuesta nuevaRespuesta,
    ...
    @Operation(summary = "Listar todas las respuestas activas",
            description = "",
            tags = { "Respuestas"})
    public ResponseEntity<Page<ListadoDeRespuestas>> listadoDeRespuestas(@PageableDefault(size = 10) Pageable paginacion){
        //return ResponseEntity.ok(respuestaRepository.findAll(paginacion).map(ListadoDeRespuestas::new)); //Listar todos los registros
        return ResponseEntity.ok(respuestaRepository.findBySolucionTrue(paginacion).map(ListadoDeRespuestas::new)); //Lista solo los registros activos
    }
    ...
    public ResponseEntity actualizarRespuesta(@RequestBody @Valid ActualizarRespuesta actualizarRespuesta){
        ...
        return ResponseEntity.ok(new DatosRespuesta(respuesta.getId(), respuesta.getMensaje(), respuesta.getFechaCreacion()));
