...
public class CursoController {
    ...
    @Operation(summary = "Registra un nuevo curso",
            ...
            tags = { "Cursos"})
    public ResponseEntity<DatosCurso> nuevoCurso(@RequestBody @Valid NuevoCurso nuevoCurso,
                                                 UriComponentsBuilder uriComponentsBuilder) {
        Curso curso = cursoRepository.save(new Curso(nuevoCurso));
        DatosCurso datosCurso = new DatosCurso(curso.getId(), curso.getNombre(), curso.getCategoria());
    ...
    @Operation(summary = "Listar todos los cursos",
            ...
            tags = { "Cursos"})
    public ResponseEntity<Page<ListadoDeCursos>> listadoDeCursos(@PageableDefault(size = 10) Pageable paginacion) {
        var page = cursoRepository.findByStatusTrue(paginacion).map(ListadoDeCursos::new);
    ...
    public ResponseEntity<?> actualizarCurso(@RequestBody @Valid ActualizarCurso actualizarCurso) {
        try {
            ...
            return ResponseEntity.ok(new DatosCurso(curso.getId(), curso.getNombre(), curso.getCategoria()));
